...
class Model {
    ...
    public function __construct() {
        ...
        $this->db = new \PDO(sprintf('%s:host=%s;dbname=%s;charset=%s', 
                $dbconfig['type'], 
                $dbconfig['host'], 
                $dbconfig['dbname'], 
                $dbconfig['charset']
                ), $dbconfig['username'], $dbconfig['password']);
        $this->fix = $dbconfig['fix'];
        $this->fix && $this->table = $this->fix . $this->table;
    }

    private function query() {
        $sql = sprintf('select %s from %s %s %s %s %s %s %s', 
                $this->field, 
                $this->table, 
                implode(' ', $this->join), 
                $this->where, 
